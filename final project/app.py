...
app.config['MYSQL_PASSWORD'] = 'Aarna@123'
...
def broadcast_transaction(data):
    ...
    for user in connected_users: 
        if str(user['PORT']) != PORT:
            requests.post('http://localhost:{}/transaction'.format(user['PORT']), json={"new_transaction": data})
...
@app.route('/update_connected_users', methods=['POST'])
