...
def get_blockchain():
    ...
    blockchain_sql = Table("blockchain", "number", "hash", "previous", "sender", "recipient", "amount", "nonce")
    for b in blockchain_sql.getall():
        blockchain.add(Block(int(b.get('number')), b.get('previous'), b.get('sender'), b.get('recipient'), b.get('amount'), int(b.get('nonce'))))
...
def sync_blockchain(blockchain):
    ...
    for block in blockchain.chain:
        blockchain_sql.insert(str(block.number), block.hash(), block.previous_hash, block.sender, block.recipient, block.amount, block.nonce)
